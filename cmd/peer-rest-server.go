...
// GetResourceMetricsHandler - returns server metrics.
func (s *peerRESTServer) GetResourceMetricsHandler(w http.ResponseWriter, r *http.Request) {
	if !s.IsValid(w, r) {
		s.writeErrorResponse(w, errors.New("invalid request"))
		return
	}
	...
	logger.LogIf(ctx, gob.NewEncoder(w).Encode(resourceMetricsMap))
}
...
func registerPeerRESTHandlers(router *mux.Router) {
	...
	subrouter.Methods(http.MethodPost).Path(peerRESTVersionPrefix + peerRESTMethodResourceMetrics).HandlerFunc(h(server.GetResourceMetricsHandler))
